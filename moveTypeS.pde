...
void normalMove() {  
  ...
  myServer.write(str(row1)+str(col1)+str(row2)+str(col2)+"0");
  takeback = true;
  firstClick = true;
  myTurn = false;
}
...
void takeBack() {  
  ...
  if (Character.isUpperCase(wLastEaten)) //if their last move ate a piece
    grid[row2][col2] = wLastEaten;    
  else grid[row2][col2] = temp;

  myServer.write(str(row2)+str(col2)+str(row1)+str(col1)+"1");
  myTurn = true;
}
...
void pawnPromo() {
  ...
  myServer.write(str(row2)+str(col2)+str(type)+str(col1)+"2");
  promo = false;
  myTurn = false;
}
