...
boolean firstClick, myTurn, takeback, promo = false;
...
char grid[][] = {
  {'R', 'N', 'B', 'K', 'Q', 'B', 'N', 'R'}, // white SIDE IS LOWERCASE
  {'P', 'P', 'P', 'P', 'P', 'P', 'P', 'P'}, 
  {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '}, 
  {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '}, 
  {' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '}, 
...
void setup() {
  ...
  brook = wrook = loadImage("whiteRook.png"); // theyre all the same shape
...
void drawPieces() {
  for (int r = 0; r < 8; r++) {
    for (int c = 0; c < 8; c++) {
      ...
      if (Character.isLowerCase(grid[r][c])) tint(0,125);
...
void mouseReleased() {
  if (myTurn) {
    if (firstClick && Character.isLowerCase(grid[mouseY/60][mouseX/60])) {
      row1 = mouseY/60;
      col1 = mouseX/60;
      firstClick = false;
